...
import JobCard, { type NYCJob } from './components/JobCard';

/* Temporary stub data – replace with API fetch later */
const sampleJobs: NYCJob[] = [
    {
    job_id: "694510",
...
const App: FC = () => {
  /* Track favorites & applied job IDs */
  const [favorites, setFavorites] = useState<Set<string>>(new Set());
  const [applied, setApplied]     = useState<Set<string>>(new Set());

  const toggleFavorite = (job: NYCJob) => {
    setFavorites((prev) => {
  ...
  const markApplied = (job: NYCJob) =>
    setApplied((prev) => new Set(prev).add(job.job_id));

  const handleLearnMore = (job: NYCJob) => {
    // route or modal here
